...
export const UserQuestionBankModal = ({ isOpen, onClose, user, onUpdated }: UserQuestionBankModalProps) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description: "Failed to update question bank access",
        variant: "destructive",
      });
    } finally {
      setSaving(false);
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[600px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Assign or revoke question bank access for {user.username}
          </DialogDescription>
        </DialogHeader>
              ...
              <ScrollArea className="h-[300px] pr-4">
                <div className="space-y-4">
                  {questionBanks.map((qb) => (
                    <div key={qb.id} className="flex items-start space-x-3 p-3 border rounded-lg">
                      <Checkbox
                        id={qb.id}
        ...
        <div className="flex items-center justify-between text-sm text-muted-foreground">
          <span>
            {userSubscriptions.length} of {questionBanks.length} question banks selected
          </span>
