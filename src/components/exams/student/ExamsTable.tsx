...
const ExamsTable = ({ filterStatus = "all" }: ExamsTableProps) => {
  ...
  const activeQuestionBankId = useAppSelector(
    (state) => state.questions.activeQuestionBankId,
  );
    ...
    try {
      ...
      dispatch(
        setCurrentExam({
          ...
          difficultyLevels: (exam.difficulty_levels || []).filter(
            (d): d is (typeof allowedDifficulties)[number] =>
              allowedDifficulties.includes(
                d as (typeof allowedDifficulties)[number],
              ),
          ),
          ...
          examType:
            exam.exam_type === "study" || exam.exam_type === "test"
              ? exam.exam_type
              : "test",
        }),
      );
      ...
      if (exam.difficulty_levels?.length) {
        ...
        const filteredDifficulties = exam.difficulty_levels.filter(
          (d: string) => allowedDifficulties.includes(d),
        ) as ("easy" | "medium" | "hard")[];
        query = query.in("difficulty", filteredDifficulties);
      }
      const fetchCount = Math.max(exam.questionCount * 2, 50);
      const { data: questions, error } = await query.limit(fetchCount);
      ...
      const formattedQuestions = (
        selectedQuestions as unknown as SupabaseQuestion[]
      ).map((q) => ({
        id: q.id,
        text: q.text,
        ...
        difficulty: allowedDifficulties.includes(
          q.difficulty as (typeof allowedDifficulties)[number],
        )
          ? (q.difficulty as (typeof allowedDifficulties)[number])
          : ("easy" as const),
        tags: [],
    ...
    return (
      <div className="flex flex-col items-center justify-center py-10 text-center">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Please select a question bank from the dropdown above to view your
          exams.
        </p>
      </div>
    );
        ...
        {exams.map((exam) => {
          ...
          return (
            <TableRow key={exam.id}>
              ...
              <TableCell>{formatDate(exam.testDate)}</TableCell>
              <TableCell>{exam.questionCount}</TableCell>
              <TableCell className="hidden md:table-cell">
                {isComplete ? formatTime(exam.timeTaken) : "-"}
              </TableCell>
              <TableCell>{isComplete ? `${exam.score}%` : "-"}</TableCell>
              <TableCell>
                <Badge
